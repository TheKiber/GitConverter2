...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЭтоWindowsСервер И НЕ ПустаяСтрока(ЭтотОбъект.Значение)
		И СтрНайти(ЭтотОбъект.Значение, "%ВерсияПлатформыХранилища%") = 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'В пути к версиям необходимо использовать <%ВерсияПлатформыХранилища%> для подстановки версии платформы.'"),
			,
