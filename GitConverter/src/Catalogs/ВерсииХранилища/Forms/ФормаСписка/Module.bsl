...
Процедура УстановитьСостояниеНаСервере(Знач ВыделенныеСтроки, Знач Состояние)
	...
	Для Каждого Ссылка Из ВыделенныеСтроки Цикл
		...
		Если Не ЗначениеЗаполнено(СостояниеСсылка) 
			ИЛИ СостояниеСсылка = Перечисления.СостоянияВерсии.ПолученCF Тогда
			ВерсияОбъект.ВыгруженыТолькоИзменения = Ложь;
		КонецЕсли;
		...
		Если УдалятьФайлы Тогда
			...
			КаталогВыгрузкиВерсий = ОбщегоНазначения.ДобавитьКонечныйРазделительПути(КаталогВыгрузкиВерсий);
			
			ФайлКомандыGit = "git_command_" + Формат(ВерсияОбъект.Код, "ЧДЦ=; ЧГ=0") + ?(ЭтоWindowsСервер, ".bat", ".sh");
			ИмяФайлКомментария = "git_comment_" + Формат(ВерсияОбъект.Код, "ЧДЦ=; ЧГ=0") + ".txt";
			Файл = Новый Файл(ФайлКомандыGit);
...
Функция ПроверитьНеобходимостьУдалятьФайлыВерсии(ВерсияОбъект, НовоеСостояние)
	...
	Если НовоеСостояние = Перечисления.СостоянияВерсии.ПустаяСсылка() Тогда
		Возврат Истина;
	ИначеЕсли (ВерсияОбъект.Состояние = Перечисления.СостоянияВерсии.ЗагруженаВEDT 
		ИЛИ ВерсияОбъект.Состояние = Перечисления.СостоянияВерсии.ПомещаетсяВGit
		ИЛИ ВерсияОбъект.Состояние = Перечисления.СостоянияВерсии.ПомещенаВGit)
		И НовоеСостояние <> Перечисления.СостоянияВерсии.ЗагруженаВEDT Тогда 
		Возврат Истина;
	КонецЕсли;
