...
Процедура ПередЗаписью(Отказ)
	...
	ИмяПользователяХранилища =СокрЛП(ИмяПользователяХранилища);
	КаталогВыгрузкиВерсий = ОбщегоНазначения.ПолноеИмяКаталога(КаталогВыгрузкиВерсий, Истина, Ложь);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЗначениеЗаполнено(АдресРепозиторияGit) ИЛИ НЕ ОбщегоНазначенияКлиентСервер.ЭтоHTTPАдрес(АдресРепозиторияGit) Тогда
