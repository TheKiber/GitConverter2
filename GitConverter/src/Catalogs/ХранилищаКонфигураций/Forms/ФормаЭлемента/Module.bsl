...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	КаталогВременныхФайлов = ТекущийОбъект.КаталогВыгрузкиВерсий + "ReportDB" + ПолучитьРазделительПути();
	ИмяФайлаЛога = ОбщегоНазначения.ИмяФайлаЛога(КаталогВременныхФайлов, "");
...
Процедура ОбновитьСостояниеНаСервере()
	...
	ИначеЕсли ЗначениеЗаполнено(Объект.ИдентификаторЗадания) Тогда
		СостояниеЗадания = "";
		РегЗадание = РегламентныеЗадания.НайтиПоУникальномуИдентификатору(
			Новый УникальныйИдентификатор(Объект.ИдентификаторЗадания));
...
Процедура ЗапуститьКонвертациюНаСервере()
	...
	Задание = ОбщегоНазначения.ВыполнитьВФоне(
		...
		Объект.Ссылка);
	СостояниеЗадания = СтрШаблон(НСтр("ru = 'Выполняется конвертация с %1'"), Задание.Начало);
...
Функция УстановитьАдресРепозиторияGitНаКлиенте(ВеткаСуществует)
	...
	Если ЗначениеЗаполнено(Результат.Лог) Тогда
		...
		КонецЕсли;
		ТекстЛога = Новый ТекстовыйДокумент;
		ТекстЛога.УстановитьТекст(Результат.Лог);
		ТекстЛога.Показать(ЗаголовокЛога, "git_origin_connect_log.txt");
...
Процедура СоздатьРепозиторийGitНаСервереОтвет(Ответ, ДополнительныеПараметры) Экспорт
	...
	ТекстЛога = Новый ТекстовыйДокумент;
	ТекстЛога.УстановитьТекст(Результат.Лог);
	ТекстЛога.Показать(НСтр("ru = 'Лог создания репозитория'"), "GitRepoInit_log.txt");
КонецПроцедуры
...
Функция ЕстьНастройкиGitLFS()
	...
	Если ЗначениеЗаполнено(Объект.ЛокальныйКаталогGit) Тогда
		ИмяФайлаАтрибутов = ОбщегоНазначения.ДобавитьКонечныйРазделительПути(Объект.ЛокальныйКаталогGit)
			+ ".gitattributes";
