...
Функция ПолучитьПараметрыОбработкиВерсии(Знач ВерсияХранилища) Экспорт
	...
	Параметры = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Хранилище, "КаталогВыгрузкиВерсий, ВерсияПлатформыХранилища,
		...
		|ИмяПроектаEDT, ВерсияEDT, ОтключитьЛогирование,
	...
	Параметры.КаталогВыгрузкиВXML = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(Параметры.КаталогВыгрузкиВXML);
...
Процедура ВыгрузитьВерсиюВXML(ПараметрыВерсии, ПараметрыХранилища) Экспорт
	...
	Попытка
		...
		Контекст.ВыгрузкаИзменений = Ложь;
		Если Не ТребуетсяПолнаяВыгрузкаВерсииВXML(ПараметрыВерсии, ПараметрыХранилища) Тогда
			...
			Если ФайлНайденИСкопирован Тогда
				...
				Иначе
					Контекст.ВыгрузкаИзменений = Истина;
		...
		Если Не Контекст.ВыгрузкаИзменений Тогда // Требуется полная выгрузка.
		...
		Если Контекст.ВыгрузкаИзменений Тогда
		...
		ЗначенияРеквизитов.Вставить("ВыгрузкаИзменений", Контекст.ВыгрузкаИзменений);
...
Функция ТребуетсяПолнаяВыгрузкаВерсииВXML(ПараметрыВерсии, ПараметрыХранилища, КодПредыдущейПолнойВыгрузкиXML = Неопределено)
...
Процедура ЗаписатьПричинуПолнойВыгрузкиВXML(ПараметрыВерсии, Причина)
	ЗаписатьВЛогВерсии(ПараметрыВерсии, 3, СтрШаблон("ru = 'Причина полной выгрузки XML: %1'", Причина));
КонецПроцедуры

Функция СкопироватьФайлСостоянияПредыдущейВерсии(Контекст, ПолноеИмяФайлаМетаданныхПредыдущейВерсии)
	...
	Если ПредыдущаяВерсия.Состояние = Перечисления.СостоянияВерсии.ВыгруженаВXML
		...
		ПолноеИмяКопируемогоФайла = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(ПредыдущаяВерсия.КаталогВременныхФайлов) + "ConfigDumpInfo.xml";
...
Процедура ДовыгрузитьКореньВерсииВXML(Контекст)
	Если Не Контекст.ИспользуетсяCLI
		Или ОбщегоНазначения.ФайлСуществует(Контекст.КаталогВыгрузкиВXML + "Configuration.xml") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьВерсиюВEDT(ПараметрыВерсии, ПараметрыХранилища) Экспорт
	...
	Попытка
		...
		Если Контекст.ИспользуетсяCLI Тогда
			ИмпортироватьВерсиюВEDTCLI(Контекст, ВерсияПлатформыДляEDT(Контекст));
		Иначе
			ИмпортироватьВерсиюВEDTRing(Контекст, ВерсияПлатформыДляEDT(Контекст));
		КонецЕсли;
		...
		ИмяФайлаПриемника = Контекст.КаталогФайловПроекта + "ConfigDumpInfo.xml";
		Файл = Новый Файл(ИмяФайлаИсточника);
		Если Файл.Существует() Тогда
			СоздатьКаталогиВИерерахии(Контекст.КаталогФайловПроекта);
			КопироватьФайл(ИмяФайлаИсточника, ИмяФайлаПриемника);
		КонецЕсли;
		
		ПрочитатьСтруктуруВыгрузкиВерсии(ПараметрыВерсии.Ссылка, Контекст);
	Исключение
		Справочники.ВерсииХранилища.ВозниклаОшибка(ПараметрыВерсии.Ссылка, ИнформацияОбОшибке());
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ИмпортироватьВерсиюВEDTCLI(Параметры, ВерсияПлатформыДляEDT)
	СтрокаКоманды = "1cedtcli -data %РабочийКаталог% -command import %БазовыйПроект% --project ""%КаталогПроектаEDT%"" --configuration-files ""%КаталогВыгрузкиВXML%"" ";
	...
	Если Прав(Параметры.КаталогВыгрузкиВXML, 1) = ПолучитьРазделительПути() Тогда
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "%КаталогВыгрузкиВXML%", Лев(Параметры.КаталогВыгрузкиВXML, СтрДлина(Параметры.КаталогВыгрузкиВXML)-1));
	Иначе
		СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "%КаталогВыгрузкиВXML%", Параметры.КаталогВыгрузкиВXML);
	КонецЕсли;
	...
	СтрокаКоманды = СтрЗаменить(СтрокаКоманды, "%ВерсияПроектаEDT%", ВерсияПлатформыДляEDT);
	...
	ОбщегоНазначения.УдалитьФайлИлиКаталог(Параметры.РабочийКаталогEDT);
	ОбщегоНазначения.УдалитьФайлИлиКаталог(Параметры.КаталогПроекта);
...
Процедура ИмпортироватьВерсиюВEDTRing(Параметры, ВерсияПлатформыДляEDT)

	СтрокаКоманды = "ring edt workspace import %БазовыйПроект% --workspace-location ""%РабочийКаталог%"" --configuration-files ""%КаталогВыгрузкиВXML%"" --project ""%КаталогПроектаEDT%"" --version %ВерсияПроектаEDT%";
...
Процедура СохранитьФайлИнформацииВыгрузки(Параметры) Экспорт
	...
	КаталогПлатформыВыгрузкиXML = КаталогПлатформыВыгрузкиXML(Параметры);
	...
	| /DumpConfigToFiles ""%КаталогВыгрузкиВXML%"" -configDumpInfoOnly
	| /DumpResult ""%ИмяФайлаРезультатов%""";
	
	СтрокаКоманды = СтрШаблон("""%1"" /@ ""%2""",
		КаталогПлатформыВыгрузкиXML + "1cv8",
		Параметры.ФайлПараметровПакетнойОперации);
	...
	СтрокаПараметров = СтрЗаменить(СтрокаПараметров, "%КаталогВыгрузкиВXML%", Параметры.КаталогВыгрузкиВXML);
...
Функция ИмяФайлаЛогаGit(Контекст) Экспорт
	Возврат ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(Контекст.КаталогВыгрузкиВерсий)
...
Функция ВыбратьВерсиюДляОбработки(ПараметрыХранилища, СостояниеДоНачала, ПредыдущаяОбработаннаяВерсия = Неопределено) Экспорт
	...
	Если СостояниеДоНачала = СостояниеПолученCF // Это задание по выгрузке в XML.
		...
		// 4. Версия требует частичную выгрузку в XML. В этом случае возвращается Неопределено,
		...
		ПерваяВерсияКотораяТребуетПолнуюВыгрузкуXML = Неопределено; // Приоритет 3.
		...
		Для Каждого ПараметрыВерсии Из Таблица Цикл
			Если ПараметрыВерсии.Состояние <> СостояниеПолученCF Тогда
				...
				Если ПараметрыВерсии.Состояние = СостояниеВыгружаетсяВXML Тогда
			...
			КонецЕсли;
			Если СостоянияВКоторыхЕстьXML.Найти(ПараметрыПредыдущейВерсии.Состояние) <> Неопределено Тогда
				Возврат ПараметрыВерсии; // Приоритет 2 тоже можно возвращать сразу, т.к. версии идут по порядку и лучше не найдем.
			КонецЕсли;
			// Определяем первую версию приоритета 3.
			Если ПерваяВерсияКотораяТребуетПолнуюВыгрузкуXML = Неопределено
				И ТребуетсяПолнаяВыгрузкаВерсииВXML(ПараметрыВерсии, ПараметрыХранилища, КодПредыдущейПолнойВыгрузкиXML) Тогда
				ПерваяВерсияКотораяТребуетПолнуюВыгрузкуXML = ПараметрыВерсии;
			КонецЕсли;
			ПараметрыПредыдущейВерсии = ПараметрыВерсии;
		КонецЦикла;
		
		// Приоритет 3 можно возвращать только когда понятно, что версий приоритета 2 нет.
		Если ПерваяВерсияКотораяТребуетПолнуюВыгрузкуXML <> Неопределено Тогда
			Возврат ПерваяВерсияКотораяТребуетПолнуюВыгрузкуXML;
		КонецЕсли;
		
		Возврат Неопределено; // Ждем появления подходящих версий (либо вообще нет версий, либо если версии приоритета 4 - см. выше).
	КонецЕсли;
