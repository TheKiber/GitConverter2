	...
	Если Параметры.ВыгружатьИзменения Тогда
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			...
			Попытка
				...
				Если Состояние = Перечисления.СостоянияВерсии.ПолученCF
				...
				ИначеЕсли Состояние = Перечисления.СостоянияВерсии.ПомещенаВGit Тогда
					...
					Иначе
						Параметры.ВыгружатьИзменения = Ложь;
					КонецЕсли;
				ИначеЕсли Состояние = Перечисления.СостоянияВерсии.ПолучаетсяCF Тогда
...
Процедура ИмпортироватьВерсиюВEDT(ВерсияХранилища, Параметры) Экспорт
	...
	НомераВерсии = СтрРазделить(ВерсияПлатформыВыгрузкиXML(Параметры), ".");
...
Процедура СоздатьФайловуюИнформационнуюБазу(Контекст) Экспорт
	...
	Если ОбщегоНазначения.КаталогСуществует(Контекст.КаталогИБ) Тогда
		УдалитьФайлы(Контекст.КаталогИБ, "*");
	КонецЕсли;
...
Процедура ДобавитьПустоеРасширениеКонфигурации(Контекст) Экспорт
	...
	Если ОбщегоНазначения.КаталогСуществует(КаталогПустогоРасширения) Тогда
		УдалитьФайлы(КаталогПустогоРасширения, "*");
	КонецЕсли;
...
Процедура ОчиститьКаталогВерсииВФоне(ВерсияОбъект, ПараметрыХранилища) Экспорт
	...
	Попытка
		...
		Если ОбщегоНазначения.ФайлСуществует(ИмяКомандногоФайлаGit) Тогда
			УдалитьФайлы(ИмяКомандногоФайлаGit);
		КонецЕсли;
		Если ОбщегоНазначения.ФайлСуществует(ИмяФайлКомментарияGit) Тогда
			УдалитьФайлы(ИмяФайлКомментарияGit);
		КонецЕсли;
	Исключение
		ВсеКаталогиОчищены = Ложь;
		ОбщегоНазначения.ЗаписатьОшибку(
					...
					Пока ОбратныйИндекс > 0 Цикл
						...
						Если СтрНачинаетсяС(СтрокаТаблицыБаз.СекцияНРег, НРег(ВерсияОбъект.КаталогВременныхФайлов)) Тогда
							...
							Для Каждого НайденныйКаталог Из НайденныеКаталоги Цикл
								Если НайденныйКаталог.Существует() И НайденныйКаталог.ЭтоКаталог() Тогда
									УдалитьФайлы(НайденныйКаталог.ПолноеИмя);
								КонецЕсли;
							КонецЦикла;
...
//
Процедура ЗаписатьВЛогВерсии(Контекст, Уровень, Текст, Знач ИмяСобытия = Неопределено)
	Если ИмяСобытия = Неопределено Тогда
		ИмяСобытия = ИмяСобытияВерсииДляЖурнала(Контекст);
	КонецЕсли;
	ОбщегоНазначения.ЗаписатьВЛог(Контекст.ИмяФайлаЛога, Уровень, ИмяСобытия, Текст);
КонецПроцедуры
