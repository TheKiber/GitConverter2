...
Функция УдалитьКонечныйРазделительПути(Знач ПолноеИмяКаталога) Экспорт
	...
	КонецЕсли;
	Пока Истина Цикл
		ПоследнийСимвол = Прав(ПолноеИмяКаталога, 1);
...
// Удаляет файл или каталог. Если файл не может быть удален (например, он занят каким-то процессом),
// то в журнал регистрации записывается соответствующее предупреждение, а процедура завершается.
//
// Параметры:
...
Процедура ОбновитьИБ(ВерсияС = Неопределено, ВерсияПо = Неопределено) Экспорт
	...
	КонецЕсли;
	ВерсияСЧислом = ВерсияЧислом(ВерсияС);
	ВерсияПоЧислом = ВерсияЧислом(ВерсияПо);
	Если ВерсияСЧислом <= ВерсияЧислом("2.0.0.1") И ВерсияЧислом("2.0.0.1") <= ВерсияПоЧислом Тогда
