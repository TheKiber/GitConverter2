...
Процедура ПередНачаломРаботыСистемы(ПараметрыПриложения) Экспорт
	
	ПараметрыПриложения.Вставить("ПараметрЗапуска", ПараметрЗапуска);
	ОбщегоНазначенияВызовСервера.ПередНачаломРаботыСистемы(ПараметрыПриложения);
	
КонецПроцедуры

// Выполняет стандартные действия при начале работы пользователя.
Процедура ПриНачалеРаботыСистемы(ПараметрыПриложения) Экспорт
	
	Если ПараметрыПриложения.Получить("ВыполненоОбновлениеИБ") = Истина Тогда
		ПоказатьОповещениеПользователя(НСтр("ru = 'Выполнено обновлени ИБ'"), , , БиблиотекаКартинок.ОформлениеЗнакФлажок);
	КонецЕсли;
...
Процедура ПоказатьТекст(Текст, Заголовок = "", ИмяФайла = "") Экспорт
	Если ИмяФайла = "" И Заголовок <> "" Тогда
		ИмяФайла = Заголовок + ".txt";
	ИначеЕсли ИмяФайла <> "" И Заголовок = "" Тогда
		Массив = СтрРазделить(ИмяФайла, "\/", Ложь);
		Заголовок = ?(Массив.Количество() = 0, ИмяФайла, Массив[Массив.ВГраница()]);
	КонецЕсли;
	
	ТекстовыйДокумент = Новый ТекстовыйДокумент;
	ТекстовыйДокумент.УстановитьТекст(Текст);
	ТекстовыйДокумент.Показать(Заголовок, ИмяФайла);
КонецПроцедуры
